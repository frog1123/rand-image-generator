...
pub fn log(img: &RgbImage, options: &CustomizationOptions, time: Duration) {
  ...
  println!(
    "{icon} | created img in {time:.3?}: width: {width}, height: {height}, variant: {variant}, bg_color: {bg_color:?}, nose_type: {nose_type} body_color: {body_color:?}, eye_color: {eye_color:?}, mouth_accessory: {mouth_accessory}, hat_accessory: {hat_accessory}", 
    icon="✔".green(),
    time=time,
    width=width,
    height=height,
